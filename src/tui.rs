...
fn render_output(f: &mut Frame, app: &TuiApp, area: Rect) {
    ...
    let mut display: Vec<Line> = app.lines.iter()
    ...
    let scroll_from_top = if app.auto_scroll {
        ...
        total.saturating_sub(inner_height).saturating_sub(app.scroll as usize)
    };

    let title = if app.busy { " output (running…) " } else { " output " };
