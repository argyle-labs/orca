...
pub fn get_ignored(root_name: &str) -> HashSet<String> {
    match root_name {
        "rebuy" => ["node_modules", ".git", ".next", "dist", "build", "vendor", "www", "docs"]
            ...
            .map(|s| s.to_string())
            .collect(),
        "brain" => [".git", "logs", "memory", "plugins", ".trash", "node_modules"]
            .iter()
            .map(|s| s.to_string())
            .collect(),
        _ => HashSet::new(),
...
fn find_single_file(nodes: &[TreeNode]) -> Option<TreeNode> {
    for node in nodes {
        ...
        if let Some(ref children) = node.children {
            if let Some(f) = find_single_file(children) {
                return Some(f);
            }
        }
    }
...
pub fn collect_all_files(nodes: &[TreeNode]) -> Vec<TreeNode> {
    ...
    for node in nodes {
        match node.node_type {
            ...
            NodeType::Dir => {
                files.extend(collect_all_files(node.children.as_deref().unwrap_or_default()))
            }
        }
    }
    files
