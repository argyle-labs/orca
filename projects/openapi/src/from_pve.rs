...
mod tests {
                                ...
                                "GET": {
                                    ...
                                    "parameters": {
                                        ...
                                        "properties": {
                                            ...
                                            "lastentries": {"type": "integer", "optional": 1, "minimum": 1}
                                        }
                                    },
                                    "returns": {
                                        "type": "array",
    ...
    #[test]
    fn strip_prefix_handles_var_decl() {
        let raw = "var apiSchema = [];";
        assert_eq!(strip_apidoc_prefix(raw), "[]");
        let raw2 = "[1,2,3]";
        assert_eq!(strip_apidoc_prefix(raw2), "[1,2,3]");
    }

    #[test]
    fn strip_prefix_handles_const_and_js_trailer() {
        // Proxmox 8.x ships `const apiSchema = [...];` followed by JS UI
        // code in the same file.
        let raw =
            "const apiSchema = [\n  {\"x\": 1}\n]\n;\nlet method2cmd = {};\nExt.onReady(...);";
        let got = strip_apidoc_prefix(raw);
        assert_eq!(got, "[\n  {\"x\": 1}\n]");
    }

    #[test]
    fn strip_prefix_balances_nested_brackets_with_strings() {
        // Brackets inside string literals must not affect depth count.
        let raw = r#"const apiSchema = [{"s": "][["}];trailer"#;
        let got = strip_apidoc_prefix(raw);
        assert_eq!(got, r#"[{"s": "][["}]"#);
    }

    #[test]
    fn strip_prefix_respects_escaped_quote_in_string() {
        let raw = r#"[{"s": "a\"]"}];trailer"#;
        let got = strip_apidoc_prefix(raw);
        assert_eq!(got, r#"[{"s": "a\"]"}]"#);
    }

    #[test]
    fn parses_three_paths_with_correct_methods() {
        let spec = parse_str(TINY).unwrap();
        let paths = &spec.paths.paths;
        assert!(paths.contains_key("/nodes"));
        assert!(paths.contains_key("/nodes/{node}"));
        assert!(paths.contains_key("/nodes/{node}/journal"));
        let ReferenceOr::Item(journal) = &paths["/nodes/{node}/journal"] else {
            panic!("expected item");
        };
        assert!(journal.get.is_some());
        assert!(journal.post.is_none());
    }

    #[test]
    fn path_param_is_emitted_as_path_parameter() {
        let spec = parse_str(TINY).unwrap();
        let ReferenceOr::Item(journal) = &spec.paths.paths["/nodes/{node}/journal"] else {
            panic!();
        };
        let op = journal.get.as_ref().unwrap();
        let has_node_path = op.parameters.iter().any(|p| match p {
            ReferenceOr::Item(Parameter::Path { parameter_data, .. }) => {
                parameter_data.name == "node"
            }
            _ => false,
        });
        assert!(has_node_path, "node must land in path params");
    }

    #[test]
    fn non_path_params_on_get_become_query() {
        let spec = parse_str(TINY).unwrap();
        let ReferenceOr::Item(journal) = &spec.paths.paths["/nodes/{node}/journal"] else {
            panic!();
        };
        let op = journal.get.as_ref().unwrap();
        let has_since = op.parameters.iter().any(|p| {
            matches!(p, ReferenceOr::Item(Parameter::Query { parameter_data, .. }) if parameter_data.name == "since")
        });
        assert!(has_since, "since must land in query params");
    }

    #[test]
    fn optional_inverts_to_not_required() {
        let spec = parse_str(TINY).unwrap();
        let ReferenceOr::Item(journal) = &spec.paths.paths["/nodes/{node}/journal"] else {
            panic!();
        };
        ...
        for p in &op.parameters {
            ...
            if parameter_data.name == "since" || parameter_data.name == "lastentries" {
                assert!(
    ...
    #[test]
    fn post_method_routes_params_to_request_body() {
        let raw = r#"[
            {
                "path": "/foo",
                "info": {
        ...
        let spec = parse_str(raw).unwrap();
        let ReferenceOr::Item(item) = &spec.paths.paths["/foo"] else {
            panic!();
        };
        ...
        assert!(body.required, "required=true when any field is required");
        let media = &body.content["application/json"];
        let ReferenceOr::Item(Schema {
            schema_kind: SchemaKind::Type(Type::Object(obj)),
            ..
        }) = media.schema.as_ref().unwrap()
        else {
            panic!("body schema not object");
        };
        assert!(obj.properties.contains_key("name"));
        assert!(obj.properties.contains_key("comment"));
        assert_eq!(obj.required, vec!["name".to_string()]);
    }

    #[test]
    fn returns_lands_under_200_application_json() {
        let spec = parse_str(TINY).unwrap();
        let ReferenceOr::Item(nodes) = &spec.paths.paths["/nodes"] else {
            panic!();
        };
        let resp = &nodes.get.as_ref().unwrap().responses;
        let ReferenceOr::Item(r) = &resp.responses[&StatusCode::Code(200)] else {
            panic!();
        };
    ...
    #[test]
    fn output_passes_progenitor_normalize_cleanly() {
        let mut spec = parse_str(TINY).unwrap();
        let report = crate::normalize::for_progenitor(&mut spec);
        // operationIds already populated by build_operation, so the
        // synth pass should be a no-op.
        assert!(
            report.synthesized_ids.is_empty(),
            "ids should be pre-populated"
        );
    }
}
