...
}): Promise<T.FsBrowseResponse> {
  const { path, allowAll } = params;

  return request<T.FsBrowseResponse>('GET', '/api/fs/browse', {
    query: { path, allowAll },
  });
}
...
export async function listGithubRepos(params: {
  ...
  const { type, sort, per_page } = params;

  return request<unknown>('GET', '/api/github/repos', {
    query: { type, sort, per_page },
  });
}
export async function listGithubIssues(params: {
  owner: string; // Repository owner
  repo: string; // Repository name
  state?: string; // Issue state: open | closed | all
  per_page?: number; // Results per page
}): Promise<unknown> {
  const { state, per_page } = params;

  return request<unknown>('GET', `/api/github/repos/${params.owner}/${params.repo}/issues`, {
...
export async function listGithubPRs(params: {
  ...
  return request<unknown>('GET', `/api/github/repos/${params.owner}/${params.repo}/pulls`, {
    query: { state, per_page },
  });
}
...
export async function installPlugin(params: {
  ...
  return request<T.OkResponse>('POST', '/api/plugins', {
    body: params.body,
  });
}
...
export async function setPluginData(params: {
  id: string; // Plugin ID
  key: string; // Data key
  body: T.SetPluginDataRequest;
}): Promise<T.OkResponse> {
  return request<T.OkResponse>('PUT', `/api/plugins/${params.id}/data/${params.key}`, {
    body: params.body,
  });
}
export async function deletePluginData(params: {
  id: string; // Plugin ID
  key: string; // Data key
}): Promise<T.OkResponse> {
