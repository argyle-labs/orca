...
export function getPalette(): Palette   { return _palette; }
export function getMode(): Mode         { return _mode; }
export function getFontSize(): FontSize { return _fontSize; }

export function setPalette(p: Palette) {
  _palette = p;
  try { localStorage.setItem(PALETTE_KEY, p); } catch { /* SSR */ }
}

export function setMode(m: Mode) {
  _mode = m;
  try { localStorage.setItem(MODE_KEY, m); } catch { /* SSR */ }
}
...
export function setFontSize(s: FontSize) {
  _fontSize = s;
  try { localStorage.setItem(FONT_SIZE_KEY, s); } catch { /* SSR */ }
}
