-- Phase A (EXPAND) of the v7-id program for `doc_roots`.
...
ALTER TABLE doc_roots ADD COLUMN uuidv7 TEXT;
UPDATE doc_roots SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_doc_roots_uuidv7 ON doc_roots(uuidv7);
CREATE TRIGGER IF NOT EXISTS doc_roots_uuidv7_autofill
AFTER INSERT ON doc_roots
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE doc_roots SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
