-- Phase A (EXPAND) of the v7-id program for `learning_progress`.
...
ALTER TABLE learning_progress ADD COLUMN uuidv7 TEXT;
UPDATE learning_progress SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_learning_progress_uuidv7 ON learning_progress(uuidv7);
CREATE TRIGGER IF NOT EXISTS learning_progress_uuidv7_autofill
AFTER INSERT ON learning_progress
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE learning_progress SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
