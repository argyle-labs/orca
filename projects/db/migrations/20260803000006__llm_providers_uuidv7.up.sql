-- Phase A (EXPAND) of the v7-id program for `llm_providers`.
...
ALTER TABLE llm_providers ADD COLUMN uuidv7 TEXT;
UPDATE llm_providers SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_llm_providers_uuidv7 ON llm_providers(uuidv7);
CREATE TRIGGER IF NOT EXISTS llm_providers_uuidv7_autofill
AFTER INSERT ON llm_providers
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE llm_providers SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
