-- Phase A (EXPAND) of the v7-id program for `plugin_data`.
...
ALTER TABLE plugin_data ADD COLUMN uuidv7 TEXT;
UPDATE plugin_data SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_plugin_data_uuidv7 ON plugin_data(uuidv7);
CREATE TRIGGER IF NOT EXISTS plugin_data_uuidv7_autofill
AFTER INSERT ON plugin_data
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE plugin_data SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
