-- Phase A (EXPAND) of the v7-id program for `secrets`.
...
ALTER TABLE secrets ADD COLUMN uuidv7 TEXT;
UPDATE secrets SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_secrets_uuidv7 ON secrets(uuidv7);
CREATE TRIGGER IF NOT EXISTS secrets_uuidv7_autofill
AFTER INSERT ON secrets
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE secrets SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
