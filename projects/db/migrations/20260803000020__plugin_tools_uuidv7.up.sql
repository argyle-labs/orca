-- Phase A (EXPAND) of the v7-id program for `plugin_tools`.
...
ALTER TABLE plugin_tools ADD COLUMN uuidv7 TEXT;
UPDATE plugin_tools SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_plugin_tools_uuidv7 ON plugin_tools(uuidv7);
CREATE TRIGGER IF NOT EXISTS plugin_tools_uuidv7_autofill
AFTER INSERT ON plugin_tools
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE plugin_tools SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
