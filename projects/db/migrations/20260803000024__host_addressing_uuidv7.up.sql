-- Phase A (EXPAND) of the v7-id program for `host_addressing`.
...
ALTER TABLE host_addressing ADD COLUMN uuidv7 TEXT;
UPDATE host_addressing SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_host_addressing_uuidv7 ON host_addressing(uuidv7);
CREATE TRIGGER IF NOT EXISTS host_addressing_uuidv7_autofill
AFTER INSERT ON host_addressing
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE host_addressing SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
