-- Phase A (EXPAND) of the v7-id program for `config_schemas`.
...
ALTER TABLE config_schemas ADD COLUMN uuidv7 TEXT;
UPDATE config_schemas SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_config_schemas_uuidv7 ON config_schemas(uuidv7);
CREATE TRIGGER IF NOT EXISTS config_schemas_uuidv7_autofill
AFTER INSERT ON config_schemas
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE config_schemas SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
