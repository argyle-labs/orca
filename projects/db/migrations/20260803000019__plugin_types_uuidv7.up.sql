-- Phase A (EXPAND) of the v7-id program for `plugin_types`.
...
ALTER TABLE plugin_types ADD COLUMN uuidv7 TEXT;
UPDATE plugin_types SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_plugin_types_uuidv7 ON plugin_types(uuidv7);
CREATE TRIGGER IF NOT EXISTS plugin_types_uuidv7_autofill
AFTER INSERT ON plugin_types
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE plugin_types SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
