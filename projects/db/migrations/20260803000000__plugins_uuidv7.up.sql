-- Phase A (EXPAND) of the v7-id program for `plugins`.
...
ALTER TABLE plugins ADD COLUMN uuidv7 TEXT;
UPDATE plugins SET uuidv7 = uuidv7() WHERE uuidv7 IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_plugins_uuidv7 ON plugins(uuidv7);
CREATE TRIGGER IF NOT EXISTS plugins_uuidv7_autofill
AFTER INSERT ON plugins
WHEN NEW.uuidv7 IS NULL
BEGIN
    UPDATE plugins SET uuidv7 = uuidv7() WHERE rowid = NEW.rowid;
END;
