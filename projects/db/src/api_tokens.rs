...
/// the plaintext token. Returns the stored summary (without the hash).
pub fn insert(
    conn: &Connection,
    id: &str,
    name: &str,
    ...
    conn.execute(
        "INSERT INTO api_tokens (id, name, token_hash, role, created_at, expires_at)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
        params![id, name, token_hash, role, created_at, expires_at],
    )?;
    Ok(ApiToken {
        id: id.to_string(),
        name: name.to_string(),
...
pub fn list(conn: &Connection) -> Result<Vec<ApiToken>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, role, created_at, last_used_at, expires_at
...
pub fn find_by_hash(conn: &Connection, token_hash: &str) -> Result<Option<ApiTokenLookup>> {
    let r = conn
        .query_row(
            "SELECT id, name, role, expires_at FROM api_tokens WHERE token_hash = ?1",
            params![token_hash],
            |r| {
                Ok(ApiTokenLookup {
                    id: r.get(0)?,
...
mod tests {
    ...
    fn unique_name_and_hash() {
        let conn = test_conn();
        insert(&conn, "id1", "n", "h1", "admin", "t", None).unwrap();
        assert!(insert(&conn, "id2", "n", "h2", "admin", "t", None).is_err());
        assert!(insert(&conn, "id3", "n3", "h1", "admin", "t", None).is_err());
    }
}
