#![allow(clippy::disallowed_types)] // upstream Docker API passthrough — shape not owned by orca
use ::docker::{Compose, ComposeError};
use axum::{
    extract::Query,
    http::StatusCode,
...
pub async fn docker_engine_handler() -> Response {
    let status = ::docker::engine::status().await;
    let label = match status.engine {
        ::docker::Engine::Colima => "colima",
        ::docker::Engine::Desktop => "desktop",
        ::docker::Engine::None => "none",
    };
...
pub async fn docker_engine_start_handler() -> Response {
    match ::docker::engine::start().await {
