...
pub fn tool_defs() -> Value {
    json!([
        {
            "name": "run_agent",
            "description": "Delegate a task to an orca agent. The backend (LM Studio, server-side Anthropic, or delegation back to Claude Code) is selected by agent_backend_status — see the agent_backend_* tools to configure. ONLY use this when the task genuinely requires language model reasoning — summarization, explanation, drafting, inference. Do NOT use for: file reads (use read_doc), searches (use search_docs), log lookup (use search_logs), service state (use list_services), schema queries (use get_graphql_info / get_rebuy_spec), or any operation with a deterministic tool that already handles it. Deterministic tools are always preferred over LLM calls. Routing: when the resolver picks Local but LM Studio is unreachable / has no chat model loaded / fails mid-call, run_agent automatically falls back to delegating to Claude Code (returns the delegate envelope). When the resolver picks Claude and server-side Anthropic is disabled, run_agent also returns a JSON envelope { action: 'delegate_to_claude_code', ... } and the caller must invoke get_agent + Agent(general-purpose) itself. Server-side Anthropic (when enabled with a key) remains hard-fail.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "agent": {
