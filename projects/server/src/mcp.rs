...
fn get_agent(args: &Value, config: &Config) -> Result<String> {
    let name = args["name"]
        .as_str()
        ...
        if is_dir {
            ...
            if !children.is_empty() {
                nodes.push((
                    true,
                    name.clone(),
                    json!({ "name": name, "path": rel, "type": "dir", "children": children }),
                ));
            }
        } else {
            let ext = full
            ...
            if ext == "md" || ext == "mdx" {
                ...
                nodes.push((
                    false,
                    name.clone(),
                    json!({ "name": stem, "path": rel, "type": "file" }),
                ));
            }
        }
    }
...
fn list_rebuy_specs() -> Result<String> {
...
fn get_rebuy_spec(args: &Value) -> Result<String> {
    let repo = args["repo"]
        .as_str()
...
fn get_rebuy_spec_public(args: &Value) -> Result<String> {
    let repo = args["repo"]
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("repo is required"))?;
    if !validate_spec_repo(repo) {
...
fn get_rebuy_graphql_schema(args: &Value) -> Result<String> {
    let repo = args["repo"]
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("repo is required"))?;
    if !validate_spec_repo(repo) {
