...
///
/// `TARGET` is set by cargo for every build script — but a binary that silently
/// baked `unknown-target` (the old fallback) is worse than useless: it blocks
/// release-asset and plugin-asset resolution by triple, and the failure is
/// invisible until a self-update mysteriously can't find its asset (exactly how
/// mint shipped an un-updatable `unknown-target` binary). So when `TARGET` is
/// somehow absent we reconstruct the triple from the `CARGO_CFG_TARGET_*` vars
/// cargo also sets for build scripts, and shout via `cargo:warning` rather than
/// baking a poison value quietly.
