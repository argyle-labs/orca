...
mod tests {
    ...
    async fn dispatch_list_routes_and_scopes_kind() {
        ...
        let out = unit_dispatch(&format!("unit.{kind}.list"), &json!({}))
            .await
            .expect("is a unit op")
            .expect("ok");
        assert!(
    ...
    async fn dispatch_create_infers_sole_provider() {
        ...
        let out = unit_dispatch(&format!("unit.{kind}.forge"), &json!({}))
            .await
            .expect("is a unit op")
            .expect("ok");
    ...
    async fn dispatch_unknown_name_returns_none() {
        ...
        assert!(
            unit_dispatch("unit.ghost_xyz.list", &json!({}))
                .await
                .is_none()
        );
    }
    ...
    async fn dispatch_update_missing_id_errors() {
        ...
        let err = unit_dispatch(&format!("unit.{kind}.spin"), &json!({}))
            .await
            .expect("is a unit op")
    ...
    fn unit_owns_matches_only_registered_ops() {
        ...
        assert!(unit_owns(&format!("unit.{kind}.list")));
        assert!(!unit_owns(&format!("unit.{kind}.nonexistent")));
