...
async fn unraid_schema(
    ...
    if args.check_drift {
        if args.dir.is_some() {
            anyhow::bail!("`check_drift` and `dir` are mutually exclusive");
        }
    ...
    let dir = args.dir.as_deref().ok_or_else(|| {
        anyhow::anyhow!("`dir` is required when `from` is set without `check_drift`")
    })?;
...
mod tests {
    ...
    use contract::config::{Config as OrcaConfig, Model};
    use serde_json::json;
    use std::path::PathBuf;
    use std::sync::Arc;
