...
//! - `lidarr.health` — **detection.** Health issues from `/api/v3/health`.
//! - `lidarr.indexers` — configured indexers from `/api/v3/indexer`.
//! - `lidarr.test_indexers` — **remediation.** Re-test every indexer.
//! - `lidarr.system_status` — app name / version / instance.
//!
//! Lidarr speaks `/api/v3` with a `/artist` media model; the [`arr::Flavor`] is
