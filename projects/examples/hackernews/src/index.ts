 ...
 */
import { pki, Transport, type TypedValue } from '@orca/sdk';
...
async function main(): Promise<void> {
  ...
  const tick = async () => {
    ...
    try {
      ...
      process.stderr.write(`hn topstories: ${(err as Error).message}\n`);
      return;
    }
    for (const id of ids) {
      let story: HnStory;
      try {
        ...
        process.stderr.write(`hn item ${id}: ${(err as Error).message}\n`);
        continue;
      }
