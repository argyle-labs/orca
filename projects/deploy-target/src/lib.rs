...
//
//   host    = the machine the workload runs on (a free-form name: `willow`,
//             `loki`, `baldur`, …; orca does not enumerate hosts).
...
//
// So `willow`+`docker`+`dockge` and `willow`+`docker`+`cli` are two distinct
...
pub struct TargetId {
    ...
    /// hosts): `willow`, `loki`, `baldur`, `freyr`, …
    pub host: String,
    pub runtime: Runtime,
    pub kind: TargetKind,
}
...
mod tests {
    ...
    fn same_host_runtime_different_kind_are_distinct() {
        // willow + docker + dockge  vs  willow + docker + cli: same host and
