...
mod tests {
    ...
    fn notify_trust_success_materializes_peer_and_sets_trust() {
        with_db(async {
            ...
            handle_notify_trust(
                &cn,
                addr(),
                req_with_params(POD_NOTIFY_TRUST_METHOD, serde_json::json!({ "trust": true })),
            )
            .unwrap();
    ...
    fn replicate_roots_on_empty_db_returns_roots_map() {
        with_db(async {
            ...
            // Serialize to confirm the wire shape carries a `roots` object.
    ...
    fn refresh_cert_cn_mismatch_rejected_with_ca_key_present() {
        ...
        let err =
            handle_refresh_cert("peer-a", req_with_params(POD_REFRESH_CERT_METHOD, params))
                .unwrap_err();
        match prev {
