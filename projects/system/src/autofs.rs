...
/// Build the privileged [`PrivilegedOp::Apply`] for a mount set, or `None` when
/// the host already matches the store (idempotent no-op). Reads the current
/// master file (world-readable) to take-over-merge it, and diffs both files
/// against disk so only genuinely-changed files are written.
...
mod tests {
    ...
    fn merge_master_takes_over_shadowing_entry_and_preserves_foreign() {
        // baldur's real shape: an indirect /mnt/pool map + unrelated foreign
