...
mod tests {
    ...
    #[test]
    fn schedule_row_deserializes_full() {
        let json = r#"{"job":"host.backup.run","cron":"0 * * * *","args":{"path":"/data"}}"#;
        let row: ScheduleRow = serde_json::from_str(json).unwrap();
        assert_eq!(row.job, "host.backup.run");
        assert_eq!(row.cron, "0 * * * *");
        assert_eq!(row.args.unwrap()["path"], "/data");
    }

    #[test]
    fn schedule_row_args_default_to_none() {
        let json = r#"{"job":"a.b.c","cron":"@daily"}"#;
        let row: ScheduleRow = serde_json::from_str(json).unwrap();
        assert_eq!(row.job, "a.b.c");
        assert!(row.args.is_none());
    }

    #[test]
    fn schedule_row_rejects_missing_required_fields() {
        // No `cron` field ⇒ deserialization must fail (not silently default).
        let json = r#"{"job":"a.b.c"}"#;
        assert!(serde_json::from_str::<ScheduleRow>(json).is_err());
    }

    #[test]
    fn missing_args_dispatches_empty_object() {
        // Mirrors the `unwrap_or_else` fallback in `tick`.
        let row: ScheduleRow = serde_json::from_str(r#"{"job":"j","cron":"@daily"}"#).unwrap();
        let args = row.args.unwrap_or_else(|| serde_json::json!({}));
        assert_eq!(args, serde_json::json!({}));
    }

    #[test]
    fn cron_parser_rejects_garbage() {
        assert!(Schedule::parse("not a cron").is_err());
    }

    #[test]
    fn is_due_fires_when_baseline_is_stale() {
        // Baseline an hour+ before now with an @hourly schedule ⇒ due.
        let now = time::now();
        let schedule = Schedule::parse("@hourly").unwrap();
        let baseline = Timestamp::from_unix_seconds(now.unix_seconds() - 7200).unwrap();
        assert!(schedule.is_due(baseline, now));
    }
}
