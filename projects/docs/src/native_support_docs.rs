//! Native docs functions — direct callable implementations for the
//! `namespace.doc.*` tools. Replaces the former `DocsService` trait per
...
use crate::docs::{
    DocNodeKind, DocRootSummary, DocRootTree, DocTreeNodeData, SearchDocHit, SearchDocMatch,
    SearchDocsData,
};
...
pub async fn list_roots(config: &Config) -> Result<Vec<DocRootSummary>> {
    let roots = docs_mod::doc_roots(config);
    let mut out: Vec<DocRootSummary> = roots
        .iter()
        .map(|r| {
            let exists = r.path.exists();
            let doc_count = if exists {
                docs_mod::count_doc_files(&docs_mod::build_doc_tree(&r.path, &r.path, &r.ignored))
            } else {
                0
            };
            DocRootSummary {
...
pub async fn get_tree(
    ...
    let compact = docs_mod::compact_doc_tree(docs_mod::build_doc_tree(&dir, &r.path, &r.ignored));
    Ok(compact
        .iter()
        .filter_map(value_to_tree_node)
        .map(|n| node_to_data(&n))
...
pub async fn get_full_tree(raw: bool) -> Result<Vec<DocRootTree>> {
...
pub async fn read_doc(config: &Config, root: &str, path: &str, llm_format: bool) -> Result<String> {
...
pub async fn search_docs(
    ...
    let all_roots = docs_mod::doc_roots(config);
...
pub async fn list_commands() -> Result<Vec<String>> {
