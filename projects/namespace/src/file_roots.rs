...
use utils::tree::{TreeNode, build_tree_raw};
...
pub fn list_roots(config: &Config) -> Result<String> {
    ...
    entries.push(json!({
        ...
        "exists": true,
        "docs": crate::embedded::file_count()
    }));
...
pub fn get_tree(args: &Value, config: &Config) -> Result<String> {
    ...
    if root_name == "docs" {
        return Ok(serde_json::to_string_pretty(&crate::embedded::tree())?);
    }
...
pub fn read_doc(args: &Value, config: &Config) -> Result<String> {
    ...
    if root_name == "docs" {
        return crate::embedded::read(doc_path)
            .map(apply)
...
pub fn search_docs(args: &Value, config: &Config) -> Result<String> {
    ...
    let all_roots = file_roots(config);
    let roots: Vec<&FileRoot> = all_roots
        .iter()
    ...
    if filter == "all" || filter == "docs" {
        for (path, matches) in crate::embedded::search(query) {
