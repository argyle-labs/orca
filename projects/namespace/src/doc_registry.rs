...
#[derive(Serialize, Deserialize, JsonSchema)]
pub struct ListFileRootsArgs {}

#[derive(Serialize, Deserialize, JsonSchema)]
pub struct ListFileRootsOutput {
    pub roots: Vec<FileRootRegEntry>,
}

#[cfg_attr(feature = "cli", derive(clap::Args))]
#[derive(Serialize, Deserialize, JsonSchema)]
pub struct AddFileRootArgs {
    pub name: String,
    pub path: String,
...
#[derive(Serialize, Deserialize, JsonSchema)]
pub struct FileRootMutationResult {
    pub name: String,
    pub changed: bool,
}
...
) -> anyhow::Result<ListFileRootsOutput> {
    let conn = db::open_default()?;
    let roots = db::docs::list_roots(&conn)?
        .into_iter()
        .map(|r| FileRootRegEntry {
            name: r.name,
            path: r.path,
...
async fn add_doc_root(
    ...
    _ctx: &contract::ToolCtx,
) -> anyhow::Result<FileRootMutationResult> {
