...
- **Consult a KB agent for codebase context.** If you don't already know the architecture, ask before deciding. See [`DELEGATION.md`](DELEGATION.md) for routing.
- **Canonical source locations.** For authoritative type and schema locations, see [`CANONICAL_SOURCES.md`](CANONICAL_SOURCES.md).

## After changes

- **Run the appropriate validation agent.** After writing, fixing, or refactoring — run lint and typecheck for the affected project to confirm correctness before reporting done. See [`DELEGATION.md`](DELEGATION.md) for the validation agent per project.
