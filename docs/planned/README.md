...
| [storage-shares.md](storage-shares.md) | Native share management: one declarative share → reconciled NFS + SMB(+fruit) + Avahi/mDNS + wsdd for Mac/Windows/Linux |
| [secrets-identity.md](secrets-identity.md) | Unified identity + secret backends: one orca login everywhere (incl. SMB), 1Password-backed secrets, baseline password rotation |
