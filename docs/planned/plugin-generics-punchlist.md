...
| NEW-I2 | **`serve_service_plugin!` main.rs boilerplate** — identical 4-line macro call per plugin _(done: `serve_*_plugin!` macros removed; the typed `Plugin` builder is now canonical)_ | all ~20 thin plugins `main.rs:6-10` | The macro is already generic; this is NOT real duplication. A plugin template in `docs/plugin-authoring/` (or having NEW-I1 optionally emit `main.rs`) is enough. Low priority — listed only for completeness. | S | NEW-I1 |
